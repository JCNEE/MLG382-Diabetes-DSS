...
def balance_training_data(X_train, y_train, target_encoder):
    """
    Apply SMOTENC balancing to the training data only.
    NOTE: SMOTE is applied here so class_weight='balanced' is NOT also set on
    the classifiers — that would double-balance and inflate minority class weight.
    """
    print("\n" + "=" * 60)
...
def train_decision_tree(X_train, X_test, y_train, y_test, target_encoder):
    ...
    dt = DecisionTreeClassifier(
        max_depth=10,
        min_samples_leaf=20,      #  prevents tiny leaves that memorise SMOTE noise
        min_samples_split=40,     #  requires meaningful splits, reduces overfitting
        random_state=42,
    )
...
def train_random_forest(X_train, X_test, y_train, y_test, target_encoder):
    ...
    rf = RandomForestClassifier(
        n_estimators=200,         # more trees = more stable predictions for minority classes
        max_depth=15,
        min_samples_leaf=10,      # prevents leaf nodes that only contain SMOTE samples
    ...
    # enumerate so rank numbers are always 1–10 regardless of DataFrame index
...
def train_xgboost(X_train, X_test, y_train, y_test, target_encoder, y_train_original):
    ...
    xgb_model = xgb.XGBClassifier(
        n_estimators=300,          #  more rounds improve minority-class recall
        max_depth=6,
        learning_rate=0.05,        #  lower LR + more trees generalises better
        subsample=0.8,             #  row subsampling reduces overfitting on 90k rows
        colsample_bytree=0.8,      #  feature subsampling (≈22 of 28 cols per tree)
        min_child_weight=5,        #  prevents splits on very small minority groups
        random_state=42,
        eval_metric='mlogloss',
        n_jobs=-1,
    )

    # compute class weights from the original imbalanced distribution,
    # then map them onto every sample in the SMOTE-expanded y_train by class.
...
def main():
    ...
    y_train_original = data['y_train'].copy()   
